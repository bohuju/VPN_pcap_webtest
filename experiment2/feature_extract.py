...
def extract_flow_features(packets):
    ...
    # 检查是否所有包的源 IP 都相同（本地代理场景）
    all_src_ips = set(p[IP].src for p in packets)
    if len(all_src_ips) == 1:
        # 本地代理：所有 IP 相同，改用端口判断方向
        proxy_port = 8080  # 你的 mitmproxy 端口
        is_uplink = []
        for p in packets:
            if p.haslayer(TCP):
                # 源端口是代理端口 => 代理发出的包（下行）
                if p[TCP].sport == proxy_port:
                    is_uplink.append(0)
                else:
                    is_uplink.append(1)
            else:
                # 非 TCP（不会发生在代理流量中，但保留）
                is_uplink.append(1)  # 默认认为是上行
    else:
        # 正常多 IP 场景：直接用第一个包的源 IP 作为客户端
