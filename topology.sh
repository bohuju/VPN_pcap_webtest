#!/bin/bash
# ============================================================
#  Network Capture Topology Demo — 独立启动
#  拓扑演示: Internet → Gateway → SW1/SW2 → STA/Server
#  含 Probe/LCM/Capture 采集模块 + LLM Agent 分析动画
# ============================================================
set -e
...
BACKEND_PORT=8001
URL="http://localhost:${BACKEND_PORT}/topology"
...
clear
echo ""
echo -e "  ${BOLD}🔬 Network Capture Topology Demo${NC}"
echo -e "  ${DIM}Gateway-to-Server traffic observed by Probe / LCM / Capture${NC}"
echo -e "  ${DIM}LLM Agent analyzes captured data → results returned to STA${NC}"
echo ""
...
sleep 1
python3 -m uvicorn backend.main:app --host 0.0.0.0 --port ${BACKEND_PORT} &>/tmp/topology_server.log &
SERVER_PID=$!
...
echo -e "  ${DIM}Demo: Start Demo → 5-stage animation (ingress → capture → LLM → return)${NC}"
