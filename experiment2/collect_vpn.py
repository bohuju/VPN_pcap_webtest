...
def capture_with_curl_vpn():
    """每个网站独立保存"""
    
    with open("url_list.txt", 'r') as f:
        urls = [line.strip() for line in f if line.strip()]
    
    print(f"[*] 开始采集 'vpn' 流量，隧道访问https,目标数: {len(urls)}")
    
    # 创建存储目录
    os.makedirs("vpn_pcaps", exist_ok=True)
    ...
    for i, url in enumerate(urls):
        try:
            ...
            pcap_file = f"vpn_pcaps/vpn_{site_name}_{i+1:03d}.pcap"
            
            print(f"\n[{i+1}/{len(urls)}] 采集: {url}")
            print(f"  保存到: {pcap_file}")
            
            # 启动抓包(443为https默认端口）
            tcpdump_cmd = ['sudo', 'tcpdump', '-i', 'ens32', 'udp', 'and', 'port', '51820', '-w', pcap_file]
    ...
    print(f"\n[+] 'vpn' 流量采集完成！文件保存在 vpn_pcaps/ 目录\n")

if __name__ == "__main__":
    # 在运行此脚本前，请确保：
    # 1. mitmproxy关闭
    # 2. 不需要Firefox配置
    capture_with_curl_vpn()
